$(function () {
    ...
    function fullHeight(el,windowHeight){
        $(el).height(windowHeight);
    }
    function fullLineHeight(el,windowHeight){
        $(el).css('line-height',windowHeight+'px');
    }
    fullHeight('.js-header',ScreenHeight);
    fullLineHeight('.js-header',ScreenHeight);

    //кнопка меню
    $(".js-menu").on('click', function(e) {
        var elToggle = $('.js-menu-general'),
            elSlideLeft = elToggle.find('.js-slide-effect-left'),
            elSlideRight = elToggle.find('.js-slide-effect-right');
        elToggle.fadeToggle(300);
        $(this).toggleClass("active");
        elToggle.toggleClass("active");
        if($(this).hasClass('active')){
            slideEffectLeft(elSlideLeft,true,-200);
            slideEffectRight(elSlideRight,true,-200);
            $(this).closest('.main-menu').css('z-index','9998');
        }
        else{
            slideEffectLeft(elSlideLeft,false,-200);
            slideEffectRight(elSlideRight,false,-200);
            $(this).closest('.main-menu').css('z-index','auto');
        }
    });
